...
module.exports.execute = async (client, message, args) => {
  ...
  let secim = args[0];
  const embed = new MessageEmbed().setColor(client.randomColor()).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setFooter("No Mercy Milona'yı Seviyor <3");
  if (secim == "liste") {
  ...
  if (secim === "sıfırla") {
    ...
    return message.channel.send(embed.setDescription("Başarıyla belirtilen  veriler sıfırlandı!"));
  };
